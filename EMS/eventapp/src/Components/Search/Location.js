...
function Location(){
    ...
    const Search = async (e) => {
        e.preventDefault();
        try {
            const response = await UserService.getEventByloc(loc);
        ...
        <div style={{ backgroundImage: `url(${star})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center', minHeight: '80vh' ,fontFamily:'Montserrat'}}>
                    ...
                    <MDBContainer fluid className='d-flex align-items-center justify-content-center ' >
                        ...
                        <MDBCard className='m-5' style={{maxWidth: '600px'}}>
                            <MDBCardBody className='px-5'>
                                <h2 className="text-uppercase text-center mb-5">Location</h2>

                                <MDBInput wrapperClass='mb-1' label='Location' size='lg' id='loc' type='text'  value={loc}
            ...
            {showTable&&(
                <div className="container custom-container" >
                    <center>
                        <h1 style={{ fontSize: '50px',fontFamily:'Montserrat' }}>Events in {loc}</h1>
                        <div className="row">
